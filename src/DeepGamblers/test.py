...
from src.DeepGamblers.train import Arguments, save_tensors
...
def load_model_from_checkpoint(args: Arguments):
    ...
    n_classes = model_arch(input_example).shape[-1]
    deep_gambler_model = DeepGamblerModel(feature_extractor, features_dim, n_classes)
    best_weights = torch.load(os.path.join(args.output_dir, "best.pth"))
    deep_gambler_model.load_state_dict(best_weights)

    model = ModelWrapper(model=deep_gambler_model, input_dim=model_essentials["input_dim"])
