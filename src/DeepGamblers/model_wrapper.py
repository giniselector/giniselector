...
from src.DeepGamblers.loss import deep_gamblers_loss
from src.utils import eval as detect_eval

logger = logging.getLogger(__name__)


class DeepGamblerModel(nn.Module):
...
class ModelWrapper(pl.LightningModule):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.gambler_criterion = deep_gamblers_loss
    ...
    def _eval_batch(self, outputs, y, reward, prefix="train"):
        if self.current_epoch >= self.pretrain_epochs:
            loss = self.gambler_criterion(outputs, y, reward)
            logits = outputs[:, :-1]
        else:
            logits = outputs[:, :-1]
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self._eval_batch(outputs, y, self.reward, prefix="train")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        score = softmax[:, -1]
        pred = torch.argmax(logits, dim=1)
        loss = self._eval_batch(outputs, y, self.reward, prefix="val")
        return {"loss": loss, "pred": pred, "y": y, "score": score}
    ...
    def test_step(self, batch, batch_idx):
        ...
        score = softmax[:, -1]
        pred = torch.argmax(logits, dim=1)
        loss = self._eval_batch(outputs, y, self.reward, prefix="test")
        return {"loss": loss, "pred": pred, "y": y, "score": score}
