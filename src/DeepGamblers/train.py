...
from src.DeepGamblers.model_wrapper import DeepGamblerModel, ModelWrapper
...
class Arguments(BaseArguments):
    ...
    def output_dir(self):
        if self._output_dir is None:
            path = os.path.join(self.checkpoints_dir, "deep_gamblers", self.model_name, str(self.seed))
            if self.val_split > 0:
    ...
    @property
    def reward_(self):
        if "svhn" in self.model_name.lower() or "svhn" in self.dataset_name.lower() and self.reward is None:
            self.reward = 2.6
            return self.reward
        if "cifar10" in self.model_name.lower() or "cifar10" in self.dataset_name.lower() and self.reward is None:
            self.reward = 2.2
            return self.reward
...
def main(args: Arguments):
    ...
    model_arch = DeepGamblerModel(feature_extractor, features_dim, num_classes)
    model = ModelWrapper(
        model=model_arch,
        reward=args.reward_,
