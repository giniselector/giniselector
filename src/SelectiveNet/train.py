...
class Arguments(BaseArguments):
    ...
    def output_dir(self):
        if self._output_dir is None:
            path = os.path.join(
                self.checkpoints_dir, "selective_net", self.model_name, str(self.seed), f"{self.target_coverage:.2f}"
            )
            if self.val_split > 0:
...
def main(args: Arguments):
    ...
    model_arch = SelectiveNet(feature_extractor, features_dim, num_classes)
    model = ModelWrapper(
        model=model_arch,
...
"""
python -m src.SelectiveNet.lightining_train \
    --model_name vgg16_cifar10 \
    --target_coverage 0.5 \
    --config_file src/SelectiveNet/selectivenet.json \
    --seed 1
"""
