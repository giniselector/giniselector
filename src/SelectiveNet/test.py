...
from src.SelectiveNet.model_wrapper import ModelWrapper, SelectiveNet
from src.SelectiveNet.train import Arguments, save_tensors
...
def load_model_from_checkpoint(args: Arguments):
    ...
    selective_net_model = SelectiveNet(feature_extractor, features_dim, num_classes)
    selective_net_model.load_state_dict(best_weights)

    model = ModelWrapper(
        model=selective_net_model, input_dim=model_essentials["input_dim"], target_coverage=args.target_coverage
    )
...
def main(args: Arguments):
    ...
    trainer = pl.Trainer(
        default_root_dir=args.output_dir, accelerator="auto", auto_select_gpus=True, max_epochs=1, logger=False
    )

    all_results = {}
    for seed in tqdm(range(1, 11, 1), "Seed"):
        args.seed = seed
        ...
        for tc in target_coverages:
            ...
            for c in range(num_classes):
                ...
                if sum(coverage_filt) == 0:
                    logger.warning(f"No predictions for class {c} with coverage <= {tc}")
                    accs[f"acc/{tc:.2f}/{c}"] = 0
                    continue
                acc = torchmetrics.functional.accuracy(
                    preds[test_indexes][c_filt][coverage_filt],
                    targets[test_indexes][c_filt][coverage_filt],
